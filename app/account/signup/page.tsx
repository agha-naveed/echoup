...
function SignupForm() {
    ...
    const handleNextStep = async () => {
        const isStep1Valid = await trigger(["firstName", "email", "password", "gender", "date", "month", "year"]);
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        try {
            ...
            let finalProfileImageUrl = authUser?.user_metadata?.avatar_url || null;
            ...
            console.error(error);
            setError("root", { type: "manual", message: "An error occurred during signup." });
        } finally {
            setIsProcessing(false);
        }
    });
                            ...
                            <div className="flex flex-col gap-3 w-full items-center">
                                {!authUser ? (
                                    <>
                                        ...
                                        className="border border-white flex items-center py-[10px] w-full md:w-[550px] justify-center gap-3 rounded-lg cursor-pointer transition-all hover:bg-white hover:text-black" title="Signin with Google">
                                    ...
                                    <div className="flex flex-col items-center gap-2 w-full md:w-[550px] bg-white/5 border border-white/10 p-4 rounded-lg mb-4">
                                        <div className="flex items-center gap-3">
                                            {authUser.user_metadata?.avatar_url && (
                                                <Image src={authUser.user_metadata.avatar_url} alt="Profile" width={30} height={30} className="rounded-full" />
                                            )}
                                            <p className="text-sm text-gray-300">
                                                Continuing as <strong className="text-white">{authUser.user_metadata?.full_name || authUser.email}</strong>
                                            </p>
                                        </div>
                                        <button
                                            type="button"
                                    ...
                                    <div className="grid gap-3 mt-2">
                                        ...
                                        <div>
                                            <div className="relative">
                                                ...
                                                <input type="email"
                                                    disabled={!!authUser}
                                                    className={`w-full h-full bg-primary text-[17px] outline-none border ${authUser && "cursor-not-allowed bg-dark-clr! text-gray-500"} md:py-3 py-[10px] pl-11 pr-4 rounded-lg ${errors.email ? "border-red-600/80" : "border-white/20"}`}
                                                    placeholder="Email"
                                                    {...register("email")} />
                                            </div>
                                        ...
                                        <div>
                                            <div className="relative">
                                                ...
                                                <input type="password"
                                                    disabled={!!authUser}
                                                    className={`w-full h-full bg-primary text-[17px] outline-none border ${authUser && "cursor-not-allowed bg-dark-clr! text-gray-500"} md:py-3 py-[10px] pl-11 pr-4 rounded-lg ${errors.password ? "border-red-600/80" : "border-white/20"}`}
                                        ...
                                        authUser?.user_metadata?.avatar_url && (
                                            <div className="flex flex-col items-center gap-4 w-[152px]">
                                                <div className={`relative w-32 h-32 rounded-full p-1 transition-all ${selectedOption === "google" ? "bg-main-blue shadow-[0_0_20px_rgba(91,171,247,0.4)]" : "bg-transparent"}`}>
                                                    <Image src={authUser.user_metadata.avatar_url} alt="Google Profile" className="w-full h-full object-cover rounded-full" width={100} height={100} />
                                                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-white rounded-full p-1 shadow-md">
                                                        <Image src={GoogleIcon} alt="google" width={20} height={20} />
                                                    </div>
                                                </div>
                                                <p className="text-sm text-center">Use your Google Photo</p>
                        ...
                        <div className="w-full shrink-0 flex justify-center px-4">
                            <div className="shadow-[0px_2px_25px_#8b8b8b1c] bg-dark-clr grid w-full p-8 rounded-xl md:w-[550px]">
                                ...
                                <div className="relative mb-8 mt-2">
                                    ...
                                    {errors.root && <p className="text-red-600/80 text-[14px] mt-2 text-center">{errors.root.message}</p>}
                                </div>
